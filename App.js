...
export default class extends React.Component {
  ...
  constructor() {
    super();
    Alert.alert("도움말", "자기보다 작냐고 물어보면 작은게 아니다!");
    this.loadSound_PlayBGM();
  }

  ask() {
    if (
      this.state.mid == this.state.right - 1 ||
      this.state.mid - 1 == this.state.left
    ) {
    ...
    if (YorN) {
      ...
      if (this.state.askIsThat) {
        ...
        this.state.mid =
          this.state.right -
          parseInt((this.state.right - this.state.left + 1) / 2);
        this.state.Qnum == 10 ? (figureOut = true) : {}; //// 질문 10번이 끝나면 무조건 답을 찾아내게 되있는데,
      } else {
        // 이것이냐고 콕 집어 물어보는 질문에 아니라고 답하면 답을 구한 줄 모름
  ...
  Questions_10() {
    ...
    if (this.state.Qnum < 10 && (answerYes || answerNo)) {
      ...
      if (figureOut == false && this.state.Qnum < 10) {
        ...
        this.playSound(soundFigureOut);
        this.setState({
          askIsThat: false,
          GameState: "End",
        });
      } else {
        this.playSound(soundFailed);
        this.setState({
          askIsThat: false,
          GameState: "Failed",
        });
